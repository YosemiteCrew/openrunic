...
modules. Thirty-one resource types are served with `read` and `search-type`; `Patient` also accepts
`create`.
...
| `Goal`                  | `patient`, `target-date`, `lifecycle-status`                          |
| `Device`                | `patient`, `identifier`, `status`                                     |
