...
import { AssistantReadback, SILENT } from '@/components/assistant';
import type { ReadbackEnding, ReadbackState } from '@/components/assistant';
...
describe('the control that turns the voice on', () => {
  it('draws nothing where the browser has no speech at all', () => {
    ...
    /* The same answer this portal gives everywhere else for something that is
       not there. A control that exists only to say it does not work is still a
  ...
  it('turns on when it is pressed', async () => {
    const { onToggle } = show(SILENT);

    await userEvent.click(screen.getByRole('switch', { name: 'Read answers aloud' }));
...
describe('while an answer is being read', () => {
  ...
  it('says so, and offers the way to stop it', async () => {
...
describe('how the last answer ended', () => {
  ...
  it('lets the voice speak over how the last one ended', () => {
    show({ ...after('failed'), speaking: { turnId: 'turn-2', text: 'You owe nothing.' } });
