 ...
 *
 * Nothing here handles a file. OpenEMR's 270/271 was a batch-file shuffle; this
...
export function CoverageCard({
  ...
  canMoveUp,
  canMoveDown,
}: CoverageCardProps): ReactElement {
  const outcome = result?.outcome ?? coverage.lastOutcome;
  const presented = presentEligibility(outcome);
  const copay = coverage.copayAmount === null ? null : formatMoney(coverage.copayAmount);
  const remaining =
    result?.deductibleRemaining === null || result?.deductibleRemaining === undefined
      ? null
      : formatMoney(result.deductibleRemaining);
    ...
    >
      ...
      <dl className="or-coverage__facts">
        ...
        <div>
          ...
          <dd className="or-mono">{copay ? copay.text : NOT_RECORDED}</dd>
        </div>
        <div>
      ...
      <div className="or-coverage__status">
        {checking ? (
          <p className="or-body" role="status">
            Checking eligibility with {coverage.payerName}
          </p>
        ) : (
          <>
            ...
            {result && result.outcome === 'ACTIVE' ? (
              <dl className="or-coverage__benefits">
                <div>
                  <dt className="or-caption">Copay today</dt>
                  <dd className="or-mono">
                    {result.copayAmount === null
                      ? NOT_RECORDED
                      : formatMoney(result.copayAmount).text}
                  </dd>
                </div>
                <div>
                  <dt className="or-caption">Deductible remaining</dt>
                  <dd className="or-mono">{remaining ? remaining.text : NOT_RECORDED}</dd>
                </div>
              </dl>
            ) : null}
          </>
        ...
        <details className="or-coverage__history">
          ...
          <ul>
            {history.map((entry, index) => (
              <li key={`${entry.coverageId}-${index}`} className="or-small">
