...
import { createWebHostedSynthesiser, createWebHostedReadbackEgress } from '../hosted-readback.js';
import type { PlaybackHandlers } from '@openrunic/voice';

describe('createWebHostedSynthesiser', () => {
  ...
  it('returns null when endpoint is not configured', () => {
    ...
    expect(createWebHostedSynthesiser()).toBeNull();
    expect(createWebHostedReadbackEgress()).toBeNull();
  });
  ...
  it('creates a synthesiser when both endpoint and agreement are configured', () => {
    ...
    const egress = createWebHostedReadbackEgress();
