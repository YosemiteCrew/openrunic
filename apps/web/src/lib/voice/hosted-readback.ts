...
/**
 * A hosted text-to-speech synthesiser for the web (staff) app.
 *
 ...
 */
export function createWebHostedSynthesiser(): HostedSynthesiser | null {
...
export function createWebHostedReadbackEgress(): { endpoint: string; agreement: string } | null {
  const config = readHostedVoiceConfig();
  if (!config) return null;
