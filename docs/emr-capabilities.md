...
| **eCQM / quality measures**                 | **Done (library)** | `packages/quality`: CMS165 and CMS122 against the published specifications, with a three-way numerator so a patient with no reading is never counted as one who passed. Value sets are declared by VSAC URL and supplied by the deployment; a measure whose value sets are absent reports that it cannot be computed rather than a rate from a partial code list. Persistence and the reporting endpoints are the follow-up |
...
| Internationalisation                   | **Partial** | `packages/i18n` is wired into `apps/web`: the locale is resolved server-side from the reader's choice or `Accept-Language`, `<html lang>` follows it, and a Spanish catalogue ships alongside the source one. A drift test fails CI on a key the catalogue does not define. The shell, navigation, downtime notices and error boundary render from the catalogue; the clinical screens still carry their strings inline  |
