...
/**
 * THE RAIL, THE COMMAND PALETTE, AND THE WORDS A TIRED PERSON TYPES INSTEAD OF
 * THE LABEL.
 *
 * Keywords are per-language and not transliterations: a Spanish speaker looking
 * for the flow board does not type "flow". Each `.keywords` entry is a comma
 * separated list, split at render, so a translator supplies the words their own
 * readers reach for rather than translating ours one for one.
 */
