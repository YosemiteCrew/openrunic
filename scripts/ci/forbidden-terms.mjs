 ...
 *
 * Kept separate so that no line in this file spells a complete trailer.
 * Interpolating one constant keeps every alternative below readable without any
 * line being a trailer.
 */
...
export const ATTRIBUTION_PATTERN = new RegExp(
  [
    ...
    // trailer above. `<assistant> code` is the same claim whichever of the three
    // writes it, and this surface is commit messages we are about to merge
    // rather than prose.
 ...
 * KELVIN SIGN; under `i` it does not, and neither does POSIX `grep -inE`. So the
 * missing `u` is what makes the pattern mean the same thing to both rather than
 * "two implementations that agree on ASCII".
 *
 ...
 *
 * FIRST, the two dialects agree ON CONSTRUCTS. This pattern is read here through
 * `new RegExp(source, 'i')`, and it is written so that `grep -inE`, which is
 * POSIX ERE, reads it the same way. `\b`, `\d`, a quantifier or a class
 ...
 * GNU grep 3.8 under `LC_ALL=C.UTF-8`.
 *
 ...
 * claim is about. The direction is harmless: grep matching MORE means a `grep`
 * reading false-positives, never that something CI would block gets through.
 *
 ...
 * is a silently green pull request. `minCorpus` is the smallest must-block
 * corpus the caller accepts, so a corpus that came back short is red.
 */
