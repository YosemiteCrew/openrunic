...
/**
 * The ONE surface the attribution arm reads.
 *
 * `messages` is `git log --format=%B MERGE_BASE..gate-head`, so a trailer found
 * there is in a commit this pull request is about to merge. Every false
 * positive found while investigating this lived in a pull-request BODY quoting
 * an upstream project's changelog - the trailer an upstream project emits is
 * byte-identical to one of ours, and the only thing separating them is which
 * repository the commit belongs to. Scoping to `messages` makes that
 * distinction structural rather than a triage step, which is why this is a
 * constant and not a list.
 *
 * RESIDUAL, stated rather than glossed: a branch that merges upstream commits
 * puts their messages into that range too, and this arm would flag them. Feature
 * branches here do not do that and bot dependency pull requests are single
 * commit, so it is small - but it is not zero.
 */
export const ATTRIBUTION_SURFACE = 'messages';

/**
 * The assistants an attribution trailer has to name, as data on ONE line.
 *
 * Kept separate so that no line in this file spells a complete trailer. The
 * local pre-commit hook enforcing this same rule scans the STAGED DIFF, so a
 * source line carrying `co-authored-by:` followed by an assistant name cannot
 * be committed from a provisioned machine - measured, `BLOCKED: an
 * AI-attribution line appears in this commit`, exit 1. The hook is right; the
 * expression is what bends. Interpolating one constant keeps every alternative
 * below readable without any line being a trailer.
 */
const ASSISTANTS = 'claude|codex|copilot';

/**
 * AI-attribution trailers, matched on the commit-message surface only.
 *
 * IN THE CLEAR, unlike the term list, and that is not an oversight. The term
 * list is secret because publishing it publishes what it withholds; an
 * attribution trailer is a string this repository refuses to CONTAIN, so
 * writing it here costs nothing and lets the expression be read and reviewed.
 * It also cannot reuse the term list's input: {@link PATTERN_SHAPE} is a plain
 * alternation over `[A-Za-z0-9 -]`, which structurally excludes the colon this
 * needs.
 *
 * Every arm requires an ASSISTANT NAME. A bare `Co-authored-by:` is how a
 * project records a human collaborator and is not what the rule forbids -
 * matching it would make the guard refuse the ordinary case and get the guard
 * deleted. `Signed-off-by:` is untouched for the same reason.
 *
 * Ported from the three-armed local hook that has been the only thing enforcing
 * this. A hook cannot reach a commit made on a machine we did not provision, or
 * one whose committer is GitHub itself; CI runs on the pull request regardless
 * of client, machine or API, which is why the arm belongs here as well.
 */
export const ATTRIBUTION_PATTERN = new RegExp(
  [
    // `Co-authored-by: <assistant> <…>` - the trailer, only when it names one.
    String.raw`co-authored-by:.*(${ASSISTANTS})`,
    // `Generated with …` / `Generated by …`, however the line is decorated.
    String.raw`generated (with|by).*(${ASSISTANTS})`,
    // The assistant's name followed by `code`, which appears with neither
    // trailer above. Deliberately WIDER than the hook this is ported from,
    // which spelled one assistant here: `<assistant> code` is the same claim
    // whichever of the three writes it, and this surface is commit messages we
    // are about to merge rather than prose.
    String.raw`(${ASSISTANTS}) code`,
    String.raw`made with (${ASSISTANTS})`,
    // The decorated form, where the assistant is named only inside a link.
...
function runScan(argv) {
  ...
  // The SHAPE check again, on the scan path, and not only in `selftest`.
  //
  // The workflow runs `selftest` before `scan`, so a pathological pattern is
  // already red before this line is reached; but that is an ORDER OF STEPS in
  // another file, which is the same kind of guarantee this change exists to
  // stop relying on. A caller that runs `scan` without `selftest` gets the
  // check anyway.
  if (!PATTERN_SHAPE.test(pattern.source)) {
    throw new GuardError(
      'the pattern is not a plain alternation of literal words over [A-Za-z0-9 -], ' +
  ...
  for (const surface of SURFACES) {
    ...
    try {
      ...
      // The window is real rather than theoretical - the surfaces live in a
      // directory this process chdir'd into.
      //
    ...
    // The attribution arm rides the SAME read rather than opening the file
    // again. A second read is a second resolution of one name, which is the
    // defect the single-descriptor block above exists to remove - and it would
    // also let this arm run over a surface the loop never validated.
  ...
  if (findings.length === 0 && attributions.length === 0) {
    // `surfaces read` is the marker the workflow greps for before it will
    // believe an exit 0. Keep that substring in this sentence whatever else it
    // gains, or a clean scan is reported as a scan that never happened.
    process.stdout.write(
  ...
  // TWO checks, so a red run has to say WHICH. They fail for different reasons
  // and are fixed in different places: one is a word in the text, the other is
  // a trailer in a commit that has to be amended or the branch rewritten.
  ...
  if (attributions.length > 0) {
    ...
    process.stderr.write(
      '\nCommits here carry no AI-attribution footer. The line is in a commit MESSAGE, so\n' +
        'a change to the working tree will not clear it: amend the commit, or rebase the\n' +
        'branch, and force-push.\n\n' +
        `The entry number is the line within the '${ATTRIBUTION_SURFACE}' surface, which is\n` +
        'every introduced commit message concatenated in log order.\n'
    );
  }
